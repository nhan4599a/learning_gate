...
export class AuthenticationService implements IAuthenticationService {
  ...
  async signIn(model: ISignInViewModel): Promise<boolean> {
    ...
    const isPasswordCorrect = await this.#verifyPassword(user.passwordHash,
      model.password,)
  ...
  async hashPassword(password: string) {
    ...
    const passwordHashBuffer = await pbkdf2Hash(
      ...
      env.PBKDF2_HASHING_ALGORITH as HmacAlgorithms,
    )
  ...
  async verifyAccessToken(accessToken: string): Promise<boolean> {
    try {
      await jwtVerify(
accessToken, this.#jwtHashingKey, {
  ...
  async #verifyPassword(hashedPassword: string, userProvidedPassword: string) {
    ...
    const saltBuffer = storedHasedPasswordBuffer.slice(0,
      env.PASSWORD_HASHING_SALT_LENGTH - 1,)

    const hashedPasswordBuffer = storedHasedPasswordBuffer.slice(env.PASSWORD_HASHING_SALT_LENGTH,)
  ...
  #setAuthenticationCookie(content: string) {
    cookies().set(
env.AUTHENTICATION_COOKIE_KEY, content, {
      httpOnly: true,
      secure: true,
      sameSite: "strict"
    }
